...
import com.haidie.gridmember.mvp.presenter.home.CheckInPunchPresenter
 ...
 * description
 */
class CheckInPunchActivity : BaseActivity(),CheckInPunchContract.View {
    ...
    private val mPresenter by lazy { CheckInPunchPresenter() }
    override fun getLayoutId(): Int = R.layout.activity_check_in_punch
    ...
    override fun initView() {
        ...
        ivAddPic.setOnClickListener { _ ->
            AndPermission.with(this)
                ...
                .onGranted {
                    //开启相机拍照
                    PictureSelector.create(this@CheckInPunchActivity)
        ...
        initTypeView()
        llEditHouseStatus.setOnClickListener {
        ...
        tvSubmitCheck.setOnClickListener {
            ...
            mPresenter.getHouseDetailCheckInPunchData(toRequestBody(uid.toString()),toRequestBody(token),
                toRequestBody(houseId.toString()),null,toRequestBody(type),
                toRequestBody(content),toRequestBody(tvAddress.text.toString()),
                toRequestBody(tvTime.text.toString()),toRequestBody(tvHouseType.text.toString()),part)
        }
    }
    private fun initTypeView() {
        ...
        data.add("租赁")
        data.add("空置")
        mPvOptions = OptionsPickerBuilder(this@CheckInPunchActivity, OnOptionsSelectListener { options1, _, _, _ ->
            //房屋状态:0：自住,1：租赁,2：空置
            tvHouseType.text = data[options1]
        }).build()
    ...
    override fun start() {}
    override fun setHouseDetailCheckInPunchData(isSuccess: Boolean, msg: String) {
        if (isSuccess) {
            showShort("走访完成")
            finish()
        }else{
