...
import com.haidie.gridmember.mvp.presenter.home.ReturnVisitRegistrationPresenter
 ...
 * description  回访登记
 */
class ReturnVisitRegistrationActivity : BaseActivity(),ReturnVisitRegistrationContract.View {
    ...
    private val mPresenter by lazy { ReturnVisitRegistrationPresenter() }
    ...
    override fun initView() {
        ...
        tvTitle.text = "回访登记"
        etContent.hint = "回访说明"
        llEditHouseStatus.visibility = View.GONE
        ivAddPic.setOnClickListener { _ ->
            AndPermission.with(this)
                ...
                .onGranted {
                    //开启相机拍照
                    PictureSelector.create(this@ReturnVisitRegistrationActivity)
        ...
        tvSubmitCheck.setOnClickListener {
            ...
            mPresenter.getReturnVisitRegistrationData(toRequestBody(uid.toString()),toRequestBody(token),
                toRequestBody(houseId.toString()),toRequestBody(mId.toString()),toRequestBody(type),
                toRequestBody(content),toRequestBody(tvAddress.text.toString()),toRequestBody(tvTime.text.toString()),null,part)
        }
    }
    ...
    override fun start() {}
    override fun setReturnVisitRegistrationData(isSuccess: Boolean, msg: String) {
        if (isSuccess) {
            showShort("提交成功")
            finish()
        }else{
