 ...
 * description
 */
class WorkPresenter : BasePresenter<WorkContract.View>(), WorkContract.Presenter {

    override fun getOrderData(admin_id: Int, token: String, status: String, page: String, size: String) {
        checkViewAttached()
        mRootView?.showLoading()
        val disposable = RetrofitManager.service.getWorkData(admin_id, token, status, page, size)
